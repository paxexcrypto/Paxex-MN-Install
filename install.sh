...
PAXEX Masternode installer arguments:
...
echo "
 ...
 | the PAXEX Team CANNOT assist you in repairing  |::
...
echo "" && echo 'Added user "paxex"' && echo ""
sleep 1
...
# Install paxex daemon
wget $TARBALLURL
tar -xzvf $TARBALLNAME 
rm $TARBALLNAME
mv ./paxexd /usr/local/bin
mv ./paxex-cli /usr/local/bin
mv ./paxex-tx /usr/local/bin
rm -rf $TARBALLNAME

# Create .paxex directory
mkdir $USERHOME/.paxex
...
# Create paxex.conf
touch $USERHOME/.paxex/paxex.conf
cat > $USERHOME/.paxex/paxex.conf << EOL
${INSTALLERUSED}
rpcuser=${RPCUSER}
...
EOL
chmod 0600 $USERHOME/.paxex/paxex.conf
chown -R $USER:$USER $USERHOME/.paxex

sleep 1

cat > /etc/systemd/system/paxex.service << EOL
[Unit]
Description=paxexd
...
ExecStart=/usr/local/bin/paxexd -conf=${USERHOME}/.paxex/paxex.conf -datadir=${USERHOME}/.paxex
ExecStop=/usr/local/bin/paxex-cli -conf=${USERHOME}/.paxex/paxex.conf -datadir=${USERHOME}/.paxex stop
Restart=on-abort
[Install]
WantedBy=multi-user.target
EOL
sudo systemctl enable paxex.service
sudo systemctl start paxex.service
